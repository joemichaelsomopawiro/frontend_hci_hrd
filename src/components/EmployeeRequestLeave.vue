  ...
  methods: { 
    ...
    async cancelRequest(requestId) { 
      ...
      try { 
        ...
        this.showNotificationMessage(error.response?.data?.message || 'Gagal membatalkan permohonan cuti', 'error') 
      } finally { 
        this.isSubmitting = false 
      } 
    }, 
    ...
    // =================================================================
    // == PERUBAHAN UTAMA DI SINI UNTUK MEMPERBAIKI ERROR TYPEERROR ==
    // =================================================================
    getApproverInfo(request) {
        const status = request.overall_status;

        // Periksa dulu apakah request.approvedBy ada isinya atau tidak
        if (status === 'approved') {
            // Jika status approved dan ada data approver, tampilkan nama. Jika tidak, beri teks default.
            return request.approvedBy ? `Disetujui oleh ${request.approvedBy.nama_lengkap}` : 'Disetujui';
        }

        if (status === 'rejected') {
            // Lakukan hal yang sama untuk yang ditolak
            return request.approvedBy ? `Ditolak oleh ${request.approvedBy.nama_lengkap}` : 'Ditolak';
        }

        if (status === 'pending') {
            // Logika untuk pending sudah aman
            const employeeRole = request.employee?.user?.role; // Ambil role dari relasi user
    ...
    hasApprovalHistory(request) { 
      return request.approved_by || request.notes || request.rejection_reason
    }, 
