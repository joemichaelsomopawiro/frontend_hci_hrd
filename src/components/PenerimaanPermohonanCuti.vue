    ...
    <div v-else class="requests-section"> 
      ...
      <div v-else class="requests-grid"> 
        <div v-for="request in requests" :key="request.id" class="request-card" :class="`status-${request.overall_status || request.status}`"> 
          <div class="request-header"> 
            <div class="employee-info"> 
              ...
              <div class="employee-details"> 
                ...
                <p>{{ request.employee?.user?.role }}</p> 
              </div> 
            </div> 
          ...
          <div v-if="request.approved_by" class="approval-history"> 
            <h4><i class="fas fa-history"></i> Riwayat Proses</h4> 
            <div class="approval-item"> 
              <div class="approver-info"> 
                <i class="fas fa-user-check"></i> 
                <span>{{ request.approvedBy?.nama_lengkap }} ({{ request.approvedBy?.user?.role }})</span> 
              </div> 
...
export default { 
  ...
  data() { 
    return { 
      ...
      selectedStatus: 'pending', // Default to 'pending' to show actionable items first
      selectedLeaveType: '', 
  ...
  methods: { 
    ...
    async loadRequests() { 
      ...
      try { 
        ...
        // PERBAIKAN: Selalu kirim parameter 'status' jika ada nilainya.
    ...
    async confirmAction() { 
      ...
      try { 
        ...
        const response = await axios.put(this.apiUrl + endpoint, payload, {  
          headers: { 'Authorization': `Bearer ${token}` }  
        }); 
        ...
        if (response.data.success) {
            ...
            alert(response.data.message || 'Gagal memproses aksi.');
        }
      } catch (error) { 
